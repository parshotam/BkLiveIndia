...
const NewHome = ({navigation}) => {
    ...
    const [iscollaps4, setIscollaps4] = React.useState(true);
    const [iscollaps5, setIscollaps5] = React.useState(true);
    ...
    const setcollapsVal = index => {
      ...
      if (index == 4) {
        setIscollaps4(!iscollaps4);
      }
      if (index == 5) {
        setIscollaps5(!iscollaps5);
      }
    };
  ...
  const startOfYear = (curdate) => {
    ...
    const getDateData = async (newdate, ntype) => {
      ...
      const data = {report_type: newtype, date_Val: newdate};
          ...
          <>
            ...
            <DataTable rowHeight={30} key={index} >
              <DataTable.Header  style={styles.tableHeader}>
                <Text style={styles.Txt672}>{item.net_stale.type == 'National' ? 'India' :item.net_stale.type } </Text>
